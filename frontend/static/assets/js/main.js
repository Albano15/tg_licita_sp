  ...
  $(document).ready(function () {
    ...
    function dynamicActiveMenuClass(selector) {
      ...
      selector.find("li").each(function () {
        ...
        if ($(anchor).attr("href") == FileName || $(anchor).attr("href") == '') {
          $(this).addClass("activePage");
        }
      });

      // Verifica se algum `li` tem a classe `activePage` e adiciona no `li` pai.
      ...
      // Se não houver um nome de arquivo, aplica a classe ao primeiro item.
      if ("" == FileName) {
    ...
    if ($('ul.sidebar-menu').length) {
      dynamicActiveMenuClass($('ul.sidebar-menu'));
    }
